...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($_POST['edit_id'])) {
    ...
    if ($career_id && $step_number && $step_title) {
      ...
      $error = "Required: career_id, step_number, step_title.";
    }
  }
  ...
  <style>
    * {margin:0; padding:0; box-sizing:border-box;}
    ...
    .sidebar {
      ...
      background:#2f2f2f; color:#fff; padding-top:60px;
      transition:.3s; z-index:2000; overflow:auto;
    }
    .sidebar a {
      display:block; padding:14px 20px; color:#fff; text-decoration:none; transition:.3s;
    }
    .sidebar a:hover, .sidebar a.active {background:#444;}
    .sidebar.show {left:0;}
    .hamburger {
      position:absolute; left:20px; top:22px; width:28px; height:20px;
      cursor:pointer; display:flex; flex-direction:column; justify-content:space-between;
    }
    .hamburger span {
      height:3px; width:100%; background:#fff; border-radius:2px;
    }
  ...
  <table>
    ...
    <tr>
      <form method="POST">
        ...
        <td data-label="Career ID"><span class="view-mode"><?=$r['career_id']?></span><input class="edit-mode" type="number" name="career_id" value="<?=$r['career_id']?>" style="display:none;"></td>
        <td data-label="Step #"><span class="view-mode"><?=$r['step_number']?></span><input class="edit-mode" type="number" name="step_number" value="<?=$r['step_number']?>" style="display:none;"></td>
        <td data-label="Title"><span class="view-mode"><?=htmlspecialchars($r['step_title'])?></span><input class="edit-mode" type="text" name="step_title" value="<?=htmlspecialchars($r['step_title'])?>" style="display:none;"></td>
        <td data-label="Detail"><span class="view-mode"><?=nl2br(htmlspecialchars($r['step_detail']))?></span><textarea class="edit-mode" name="step_detail" style="display:none;"><?=htmlspecialchars($r['step_detail'])?></textarea></td>
